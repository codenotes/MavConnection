...
namespace SerialCommunicationNamespace
{
	...
	public:
		size_t SendMessage(uint8_t* const buffer, size_t buffer_length) { return 0; }
		size_t ReceiveMessage(uint8_t* const buffer, size_t buffer_length) { return 0; }
		ComunicationInterfaceState getInterfaceState() const { return state; }
		SerialCommunication(string ardupilot_serial_device = "/dev/ttyACM0", int baud_rate = 115200){};
		~SerialCommunication(){}
				...
				for (int16_t i = 0; i < result; i++) {
					if (mavlink_parse_char(MAVLINK_COMM_0, buffer[i], &message, &status)) {
						...
						case MAVLINK_MSG_ID_PARAM_VALUE:
							...
							break;
						default:
							break;//printf("Unhandled Received message with ID %d, sequence: %d from component %d of system %d\n", message.msgid, message.seq, message.compid, message.sysid);
						ActionParserExecutive();
						}
